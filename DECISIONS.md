...
- **Status:** **Approved** (2026-09-20, owner-approved for planning +
  Stage A only — analytical/repository assessment; no install,
  provisioning, server setup, DNS change, credential connection, or
  deployment authorized). Implementation stages B–H remain gated by
  separate per-stage owner authorizations (plan §17).
...
- **Decision (approved):** adopt **Dokploy** as an *optional,
...
- **Verification (Stage A, 2026-09-20):** architecture & repository
  assessment executed and recorded in
  `docs/deployment/dokploy-plan.md` §20 — compose manifest,
  container/Dokploy alignment, env-var inventory, named-volume
...
- **Status:** **Approved** (2026-09-20, owner-directed) — adoption
  approved for the live-AI-integration horizon; implementation is
  **PLANNED**, gated behind Dokploy staging/production stabilization
  (D-141 stages C+) and a dedicated future work authorization. Nothing
  installed, connected, or integrated in this registration.
- **Situation:** Phases 7–10 shipped the AI runtime strictly local —
  `AiProvider` boundary (D-062/D-129) with a deterministic
  `MockAiProvider` (D-053), proposal lifecycle (D-064), and
  observability — so agent context is siloed per process today. When
  agents go live, interactive context would be lost across sessions
  and apps (context siloing) without an external shared-memory layer.
- **Decision:** adopt **MystenLabs/MemWal** (Walrus Memory —
  https://github.com/MystenLabs/MemWal, owner-selected; repository
  existence and description verified 2026-09-20: portable, verifiable
  agent-memory layer storing encrypted memories on Walrus decentralized
  storage, SDK `@mysten-incubation/memwal` + relayer backend) as the
  external, encrypted, portable shared-memory layer for AI agents,
  integrated at live-AI-integration time per the repository's official
  guidance.
- **Consequences (boundaries carried into any future integration):**
  (1) D-045 — MemWal's Walrus/relayer services are EXTERNAL network
  dependencies; any connectivity requires explicit owner authorization
  and credentials (account/delegated key) never enter Git, logs, or
  docs; (2) provider-neutral seam (D-129/D-131 pattern) — the memory
  layer sits behind an injected interface with a local deterministic
  parity backend, so it stays replaceable (Phase 24 lock-in goals) and
  the battery remains air-gapped; (3) D-026/D-027 — memory writes are
  never authority: business truth and approvals stay in the canonical
  store and Phase 19 chain; (4) zero-leak redaction (D-114/D-124)
  applies before any context leaves the process; (5) owner-referenced
  future optimization workstreams "Phases 27–28" are recorded as the
  owner's stated scope — no such phase records exist in this register
...
## D-146 — Stage F context-bound owner authorization engine

- **Status:** **Approved** (2026-09-24, owner-directed).
- **Situation:** D-145 made the cutover manifest bytes a technical
  prerequisite, but the final switch still needed a machine-enforced
  OWNER half: per plan §17/§21.6 and D-139, technical clearance
  (V-01..V-09) is necessary, never sufficient.
- **Decision:** cutover is armed only against an explicit,
  single-use, context-bound, time-bounded owner authorization token,
  validated by `local/src/security/owner_approval_gate.py`:
  - **Cryptographic binding** — HMAC-SHA256 over the canonical JSON
    of (manifest_sha256 from the D-144 envelope, cutover session id,
    target environment, issued/expires logical ticks, owner nonce);
    wire format `<token_id>.<sig>` where token_id is the sha256
    commitment of the binding (recomputed + constant-time compared
    before the signature is touched).
  - **Fail-closed taxonomy** — malformed token/draft/context,
    envelope invalid or drifted, draft minted for another manifest
    (`fingerprint_mismatch`), unknown binding, invalid signature,
    malformed/absurd TTL window, `token_expired`,
    `token_not_yet_valid`, malformed nonce, and `replay_rejected`
    all refuse; every unlisted failure is also a refusal. Absence of
    a gate or token is NEVER a pass.
  - **Single-use burn** — the nonce burns through an INJECTED replay
    store under the deterministic key sha256({burn, token_id,
    nonce}); durable semantics make replay refusal survive gate
    re-instantiation.
  - **Determinism & purity** — expiry judged against an INJECTED
    logical clock only (no wall clock); zero network/subprocess/file
    I/O in the engine (AST-pinned); every outcome — GO or NO_GO —
    emits exactly one deep-redacted report to the INJECTED audit
    sink (D-121 `engine.log.v1` in production); sink failure raises
    (no quiet evaluation). Key and signature material never appear
    in any report, error, or artifact (D-124).
  - **Revocation model** — manifest regeneration, session change,
    gate re-binding, TTL expiry, or prior consumption each void
    outstanding tokens; the D-139 kill switch remains the runtime
    halt path (tokens arm, never override).
  - Spec: `docs/deployment/stage-f-owner-authorization.md` (lifecycle,
    handoff protocol, rejection taxonomy, evidence validity).
- **Boundaries preserved:** nothing authorized, nothing deployed —
  the gate is a necessary input to activation; Stage F sign-offs and
  D-139 remain the sole cutover authority.
- **Verification (2026-09-24):** battery
  `local/tests/test_owner_approval_gate.py` 19/19 ×2 (GO + burn,
  determinism, tamper/wrong-key refusal, expiry/not-yet-valid/window
  bounds, manifest/session/context mismatches, cross-instance replay,
  replay audited, key+signature+canary absent from all reports and
  errors, AST no-I/O audit, report shape contract). Full regression
  1411/1411 ×2 consecutive green across 62 modules (1392 + 19, census
  machine-reconciled identical; the first RUN1 attempt failed honestly
...
## D-153 — Stage H live cutover orchestration & owner activation record

- **Status:** **Approved** (2026-09-25, owner-directed).
- **Situation:** D-152 closed Stage G with the seal, but nothing
  EXECUTED the handoff: Stage H — the transition to ACTIVE — had no
  machine-enforced executor binding the seal, the fresh unspent
  Stage F owner token, and the pre-cutover environment assertions
  into one fail-closed activation with an immutable record.
- **Decision:** `local/scripts/stage_h_cutover_executor.py` — the
  Stage H execution engine with the H-01..H-05 rules (all fail-
  closed; ANY refusal aborts with ZERO side-effects — no transition,
  no state change, one audited `CUTOVER_ABORTED` record):
  - **H-01 seal:** the D-152 `stage_g_closure_seal.v1` must be
    present, `STAGE_G_CLOSED`, digest-recomputing, and ROOTED in the
    D-112 audit trail (kind `stage_g_closure` or digest-in-detail);
    a missing, OPEN, altered, or unrooted seal refuses instantly.
  - **H-02 owner token:** the Stage F gate verdict must be GO (the
    nonce burns at GO — a replay arrives as NO_GO and refuses), the
    draft's TTL window must COVER the activation tick (minted
    freshness, not just gate freshness), and the draft fingerprint
    must equal the seal's manifest fingerprint (a token for a
    different deployment authorizes nothing).
  - **H-03 environment:** the Dokploy target state must show every
    service running + healthy with restarts ≤ 3 and ZERO unmapped
    port exposure on backend services — read via the sanctioned
    `DokployStateAdapter` (direct argv `docker inspect` JSON, strict
    timeouts, allow-listed parameters, no shell, deep redaction
    before any string escapes, D-124).
  - **H-04 transition:** the injected transition provider flips the
    deployment state to ACTIVE; the immutable
    `stage_h_activation_record.v1` is emitted exactly once with the
    SHA-256 `activation_digest` over its canonical bytes; failed or
    unconfirmed transitions abort without a record lie.
  - **H-05 critical window:** post-activation probes are watched;
    a failure arms the ATOMIC ROLLBACK PAYLOAD
    (`stage_h_rollback_payload.v1`, the Stage E §5 RB-1 shape with
    trigger, ordering invariant, procedure, post-verification) — a
    VERDICT the operator executes, never an engine action; an
    unwatchable window arms rollback too (fail-closed, never a
    silent pass).
  - **Purity (RULES §35, AST-pinned):** the executor core is pure —
    injected providers, zero sockets, zero subprocess, zero wall
    clock; the adapter is the only host-touching surface. Public
    commitments (manifest hash, closure digest, token id) are
    restored after redaction in the ledger copy (D-146 precedent);
    the signing key and nonce NEVER enter any record.
- **Spec:** `docs/deployment/stage-h-cutover-runbook.md` —
  preconditions, offline owner token minting/injection (mint_token
  + TokenDraft), cutover steps 2–4, edge-only traffic routing,
  critical-window rollback execution, safety properties,
  post-activation monitoring.
- **Boundaries preserved:** the executor performs no live action by
  itself — a real cutover still requires the owner's explicit,
  single-use token and the operator executing the runbook; the
  engine never mutates the live stack on its own authority (D-139,
  plan §17/§21.6). Nothing provisioned, nothing activated.
- **Verification (2026-09-25):** battery
  `local/tests/test_stage_h_cutover_executor.py` 25/25 ×2 — the full
  cutover through the REAL D-152 closure runner and the REAL Stage F
  gate (mint → evaluate → burn) with deterministic, immutable
  `activation_digest`; refusal of missing/OPEN/altered/unrooted
  seals, stale/window-missed/divergent/replayed tokens, degraded
  targets, exposed ports, and failed transitions; zero-side-effect
  proof (the transition provider is provably never invoked on
  refusal); rollback arming + RB-1 payload shape; redaction scrub
...
- **Situation:** D-154 closed the infrastructure program with the
  `dokploy.completion_attestation.v1` certificate, but no engine had
  yet LIVE-CONNECTED the core services under it — the first phase of
  the Live Wiring program (MASTER_PLAN Phases 5–18) must connect the
  canonical PostgreSQL SSOT, the state/cache Redis tier, and the n8n
  orchestration webhook conduits under the verified runtime profile,
  fail-closed against any missing or tampered certificate, failed
  handshake, or credential leak.
- **Decision:** `local/scripts/live_wiring_phase5_igniter.py` — the
  Phase 5 ignition engine (IGN-01..IGN-05, all fail-closed; exactly
  ONE audited attestation per run INCLUDING aborts, which emit
  IGNITION_INCOMPLETE; any refusal aborts with zero side-effects and
  no transport probes run after an IGN-01 refusal):
  - **IGN-01 certificate gate:** the D-154 certificate is verified
    (schema, INFRASTRUCTURE_COMPLETE verdict, byte-exact
    `attestation_digest` recomputation, manifest binding to the
    Stage E fingerprint, STAGE_G_CLOSED seal and CUTOVER_EXECUTED
    activation record, D-112 rooting under kind
    `dokploy_completion_attestation`, zero chain breaks) BEFORE any
    probe runs; absence, alteration, digest drift, or unrooted rows
    refuse immediately.
  - **IGN-02 PostgreSQL SSOT:** authenticated connectivity through
    the argv-only `ArgvPsqlTransport` (fixed token lists, env-name
    connection refs, ON_ERROR_STOP, strict timeouts, stderr never
    echoed), schema readiness (33 tables / 16 engine schemas / 28
    seed rows live), and pooling invariants (max_connections −
    superuser_reserved ≥ POOL_HEADROOM 12).
  - **IGN-03 Redis:** connectivity with PING latency under the
    50 ms budget (`REDIS_LATENCY_BUDGET_MS`),
    `maxmemory-policy = noeviction`, and the isolated
    `phase5:ignition_drill` SET/GET/DEL namespace round-trip;
    credentials ride REDISCLI_AUTH inside the container namespace,
    never argv, never reports.
  - **IGN-04 n8n webhook dispatcher:** readiness through the REAL
    D-053 contracts — payload schema validation, HMAC `sha256=`
    verification with an injected secret the engine never holds
    (D-045 pattern), and the D-027 idempotency drill (replay
    returns the original record; the drill event
    `phase5-ignition-drill-0001` round-trips the injected D-027
    store — the ONLY write the engine performs).
  - **IGN-05 emission:** the canonical
    `phase5.live_wiring_attestation.v1` as a frozen dataclass with
    the SHA-256 `attestation_digest` over canonical JSON,
    deterministic for identical inputs; the engine core is pure
    (injected clock/sink/providers, AST-pinned zero I/O).
- **Purity & security:** D-151 adapter pattern — argv-only
  transports (`build_phase5_transports`), zero shell/Popen, no
  network imports in the core, deep redaction (D-124) over every
  emitted record with public commitments (`cert_digest`,
  `manifest_sha256`) restored after redaction; typed errors only,
  sanitized (no stderr, no raw payloads).
- **Report:** `docs/deployment/phase-5-live-wiring-report.md` —
  verdict, IGN rule results, connection topology, latency
  benchmarks (Redis PING 28.6 ms live; PG round-trip ≈42.5 ms
  argv-dominated), live-evidence provenance (redis ran as an
  ephemeral drill container matching the D-144 manifest service;
  the canonical compose declares redis for the Dokploy target),
  and the verified handover to Phase 6 (Notion OS sync).
- **Verification (2026-09-26):** battery
  `local/tests/test_live_wiring_phase5.py` 38/38 ×2 (pass path,
  certificate refusals, PG/Redis fail-closed, webhook
  schema/idempotency rejections, redaction scrubs, AST purity)
...
- **Boundaries preserved:** ignition evidence is not authority —
  Phases 6–18 wiring, external providers, and production
  connectivity remain owner-gated (D-045/D-139, plan §17/§21.6).
  No secrets were created, stored, or transmitted; the drill wrote
  exactly one audited event and nothing else.

## D-156 — Phase 6 live wiring ignition & Notion workspace sync verification

- **Status:** **Approved** (2026-09-26, owner-directed).
- **Situation:** D-155 ignited the core services under the D-154
  completion certificate, but the Notion Workspace Business OS layer
  — the canonical external mirror for catalog, orders, campaigns
  and SOPs — was not yet wired. Under D-139 / D-154 / D-155 / plan
  §17 / §21.6 the second Live Wiring phase must connect the Notion
  workspace under the same fail-closed discipline: no wiring on an
  unattested or drifted upstream, no unauthenticated API access,
  no schema drift, no destructive writes, no secret leakage.
- **Decision:** `local/scripts/live_wiring_phase6_igniter.py` — the
  Phase 6 ignition engine (NOT-01..NOT-05, all fail-closed; exactly
  ONE audited attestation per run INCLUDING aborts, which emit
  IGNITION_INCOMPLETE; a NOT-01 refusal runs ZERO Notion calls):
  - **NOT-01 phase5 gate:** the `phase5.live_wiring_attestation.v1`
    must be present, PHASE5_IGNITED, manifest-fingerprint-bound, and
    its canonical bytes must recompute to the SHA-256 commitment
    rooted in the D-112 ledger row (kind
    `phase5_live_wiring_attestation`) over an intact chain — a
    mismatch refuses as DRIFTED/altered before any probe.
  - **NOT-02 authentication & pacing:** `GET /users/me` through the
    injected client with the REAL canonical contract layer
    (`canonical/notion_live.py`); 401/403 classify to Class-E
    NotionAuthError (immediate refusal), timeouts/transients typed;
    a 12-op burst over the 3/s NotionPacer token bucket is PACED
    (measured waits 0.7→0.1 s), never dropped; the integration
    token never leaves the client and every escaping error string
    passes redact_notion.
  - **NOT-03 schema conformance:** the four canonical databases
    (Product Catalog, Order Pipeline, Marketing Campaigns,
    Tasks/SOPs) verified for property names + types + select
    options + relation integrity against the declared
    WorkspaceSchema — missing properties, corrupted types,
    missing options and unbound relations refuse with the
    offending database/property named.
  - **NOT-04 non-destructive probe:** a FIXED synthetic payload
    (deterministic D-027-style idempotency key = SHA-256 over the
    payload's canonical bytes) runs create → replay (MUST return
    the SAME page; a distinct page is an idempotency COLLISION) →
    read-back → archive inside one run; per-step telemetry names
    the failing leg (WRITE/REPLAY/READ/CLEANUP); the probe page is
    always archived (workspace left clean).
  - **NOT-05 emission:** the canonical
    `phase6.live_wiring_attestation.v1` as a frozen dataclass with
    the SHA-256 `attestation_digest`, deterministic for identical
    inputs; the engine core is pure (injected clock/sink/providers,
    AST-pinned zero I/O).
- **Purity & security:** D-124 deep redaction over every emitted
  record with public commitments (`phase5_digest`,
  `manifest_sha256`) restored after redaction; the engine never
  constructs a transport — the client arrives injected (`notion=`)
  with the live HTTPS transport injected INSIDE it (D-045/D-075);
  refusal details carry exception TYPE names only (never raw server
  bodies); no network imports, no os/subprocess, no shell/spawn in
  the core (AST-audited).
- **Report:** `docs/deployment/phase-6-live-wiring-report.md` —
  verdict, NOT rule results, the four-database schema map,
  verification topology, latency metrics (full in-process cycle
  0.56 ms; paced burst schedule), security posture, and the
  verified handover to Phase 7 (AI Runtime & Product Manager
  wiring).
- **Verification (2026-09-26):** battery
  `local/tests/test_live_wiring_phase6.py` 42/42 ×2 (pass path,
  NOT-01 refusals incl. drift-via-ledger, auth/timeout/401/403,
  schema drift, probe/idempotency collisions, emission contract,
...
- **Boundaries preserved:** the workspace is verified, not
  ...
  §17/§21.6). No Notion token was created, stored, or transmitted;
  the only write is the archived synthetic probe.

## D-157 — Phase 7 live wiring ignition & AI Runtime + Product Manager verification

- **Status:** **Approved** (2026-09-26, owner-directed).
- **Situation:** D-156 wired the Notion Workspace Business OS, but
  the AI Runtime and the Product Manager core loop were not yet
  proven on top of the attested surfaces. Under D-139 / D-154 /
  D-155 / D-156 / plan §17 / §21.6 the third Live Wiring phase must
  prove the runtime loads the verified profile, routes providers
  deterministically under hard caps, executes a bounded synthetic PM
  cycle with zero sensitive persistence, and refuses on any drift —
  all fail-closed and owner-gated.
- **Decision:** `local/scripts/live_wiring_phase7_igniter.py` — the
  Phase 7 ignition engine (AIR-01..AIR-05, all fail-closed; exactly
  ONE audited attestation per run INCLUDING aborts; an AIR-01
  refusal performs ZERO provider calls):
  - **AIR-01 phase6 gate:** the `phase6.live_wiring_attestation.v1`
    must be present, PHASE6_IGNITED, manifest-bound, its canonical
    bytes recomputed to the SHA-256 commitment rooted in the D-112
    ledger row (kind `phase6_live_wiring_attestation`) over an
    intact chain — drift/unrooted refuse BEFORE any provider call.
  - **AIR-02 runtime profile:** the injected census must carry
    `runtime_profile_verified: true` with Phase 5 AND Phase 6 rows
    present+VERIFIED+WIRED, and the repo-real seams
    (`canonical.ai_runtime` = ENTRY_POINTS[7],
    `canonical.ai_contracts`, `canonical.vocab`,
    `canonical.ai_proposal_lifecycle` = ENTRY_POINTS[8]) must be
    importable and consistent with the D-154 ENTRY_POINTS registry
    (a mismatch refuses as registry drift).
  - **AIR-03 bounded routing:** provider selection DETERMINISTIC
    (route resolved twice, divergence refuses); explicit
    provider/model allowlist (`mock: (mock-1,)` default — unknown
    provider/model refuses); hard caps enforced: max_tokens ≤ 2048,
    budget ≤ $1.00/cycle, ≤ 4 tool calls, ≤ 2 retries (Class-A
    transients only — auth/contract failures refuse immediately),
    15 s timeout.
  - **AIR-04 synthetic PM cycle:** fixed fixture (brief
    phase7-pm-brief-0001: کت و جلیقه / مشکی / پاییز) through
    normalize → vocabulary alignment (owner-approved CATEGORY_PAIRS
    + D-032 O/I/L-safe color codes only) → strategy draft through
    the REAL `ModelRouter` (route → guardrails → infer →
    `ai_contracts` validation; the proposal may only target the
    lifecycle root "Backlog", D-060) → content plan skeleton →
    packaging into the namespace-scoped `ScratchStore` (keys outside
    `phase7-scratch:` refuse); per-step telemetry
    (START/VOCAB/ROUTE/INFER/VALIDATE/PACK/CLEANUP); deterministic
    SHA-256 summary hash; the ONLY external write is the optional
    strictly probe-only Notion probe (create/replay-SAME-page/
    read-back/archive with an idempotency key); cleanup is mandatory
    even on abort (fail-closed hygiene).
  - **AIR-05 emission:** the canonical
    `phase7.live_wiring_attestation.v1` as a frozen dataclass with
    the SHA-256 `attestation_digest`, deterministic for identical
    inputs.
- **Purity & security:** injected router/Notion/census only (zero
  sockets, zero shell, zero wall clock — AST-pinned); prompts, tool
  inputs and provider fragments NEVER enter any emitted record
  (hashes, counts, route names, step verdicts only); D-124 deep
  redaction over every record with public commitments
  (`phase6_digest`, `manifest_sha256`) restored after redaction;
  strict data minimization in the certificate.
- **Report:** `docs/deployment/phase-7-live-wiring-report.md` —
  upstream verification method, runtime profile verification,
  routing constraints table, the measured cycle trace (mock route
  mock/mock-1, 45/49 tokens, $0.00, full cycle 3.51 ms in-process,
  summary hash c4cd60975127e8d9…), guardrails, security posture,
  and the Phase 8 (Instagram wiring) handover criteria.
- **Verification (2026-09-26):** battery
  `local/tests/test_live_wiring_phase7.py` 45/45 ×2 (pass path +
  deterministic digest, AIR-01 refusals with the zero-provider-call
  proof across four failure classes, profile/seams refusals, routing
  refusals incl. unknown provider/model and cap breaches, cycle
  refusals incl. unsafe tool, scratch scope violation, retry
  overflow, auth failure, invalid output schema, probe collision and
  cleanup failure, emission contract, redaction scrubs, AST purity)
  — the phase6 attestation built by the REAL D-156 igniter over the
  REAL D-155 igniter over the authentic Stage C→H chain, the AI leg
...
- **Boundaries preserved:** the runtime is proven, not deployed —
  ...
  (D-045/D-139, plan §17/§21.6). The cycle cost $0.00 (mock tariff);
  the only writes are the scratch artifact (deleted) and the
  archived Notion probe.

## D-158 — Phase 8 live wiring ignition & Instagram Graph API verification

- **Status:** **Approved** (2026-09-26, owner-directed).
- **Situation:** D-157 proved the AI Runtime + PM core loop, but the
  Instagram channel — the primary content-distribution surface —
  was not yet wired. Under D-045 / D-124 / D-139 / D-154 / D-157 /
  plan §17 / §21.6 the fourth Live Wiring phase must verify the
  Graph API adapter in STRICT PROBE-ONLY mode: no public publishing,
  no feed pollution, no irreversible external mutation — only read
  probes, capability checks, and deterministic container probes
  under synthetic IDs.
- **Decision:** `local/scripts/live_wiring_phase8_igniter.py` — the
  Phase 8 ignition engine (IG-01..IG-05, all fail-closed; exactly
  ONE audited attestation per run INCLUDING aborts; an IG-01
  refusal performs ZERO adapter/network calls):
  - **IG-01 phase7 gate:** the `phase7.live_wiring_attestation.v1`
    must be present, PHASE7_IGNITED, manifest-bound, its canonical
    bytes recomputed to the SHA-256 commitment rooted in the D-112
    ledger row (kind `phase7_live_wiring_attestation`) over an
    intact chain — drift/unrooted refuse BEFORE any adapter call.
  - **IG-02 runtime profile:** the injected census must mark
    Phases 5, 6 AND 7 present+VERIFIED+WIRED, and the repo-real
    Instagram seams (`canonical.instagram_adapter` (D-069/D-071),
    `canonical.instagram_contracts`, `canonical.instagram_publisher`
    (D-070), `canonical.instagram_live` (D-072), and
    `publishing.instagram` = ENTRY_POINTS[9]) must be importable
    and consistent with the D-154 ENTRY_POINTS registry.
  - **IG-03 capability profile:** required OAuth scopes
    (`instagram_basic`, `instagram_content_publish`,
    `pages_show_list`) verified present; token validity with an
    expiry margin ≥ EXPIRY_THRESHOLD_TICKS (300 logical ticks);
    capability-probe failures (401/403 carriers) refuse immediately
    (no retries); the Graph usage envelope (X-App-Usage shape)
    feeds the REAL `GraphUsageTracker` — usage at/over the 75%
    warn level refuses; adapter retries are Class-A-only, ≤ 2.
  - **IG-04 probe-only media workflow:** the caption is produced by
    the REAL Phase 7 ModelRouter (`generate_caption` →
    `caption_proposal.v1` contract-validated), then the REAL
    `validate_publish_payload` enforces local Class-B prevention
    (aspect ratio 4:5 ∈ {1:1, 4:5, 16:9}, caption ≤ 2200 chars,
    ≤ 30 hashtags, media-hash ≥ 8 chars) BEFORE any adapter call;
    a synthetic container is created (IN_PROGRESS), polled to
    FINISHED through the REAL bounded `poll_until_ready` (≤ 10
    polls; ERROR/EXPIRED terminal), VERIFY audits the adapter call
    log (ANY `publish_container` invocation = SAFETY VIOLATION
    refusal) and checks probe state collisions, CLEANUP archives
    the probe container through the probe-only seam — per-step
    telemetry (START/AUTH/CONTAINER_CREATE/STATUS_POLL/VERIFY/
    CLEANUP) and a deterministic SHA-256 summary hash; captions and
    container ids appear only as hashes.
  - **IG-05 emission:** the canonical
    `phase8.live_wiring_attestation.v1` as a frozen dataclass with
    the SHA-256 `attestation_digest`, deterministic for identical
    inputs; the engine core is pure (injected adapter/router,
    AST-pinned zero I/O; the source never calls publish_container).
- **Purity & security:** D-124 deep redaction over every emitted
  record with public commitments (`phase7_digest`,
  `manifest_sha256`) restored; the canonical adapter's `redact()`
  strips token markers (`IGQV…`/`EAAG…`/`access_token=`) from every
  escaping string; no page ids, media URLs or raw API responses in
  any record; the adapter and router arrive injected with the
  transport injected INSIDE them (D-045/D-075); no credentials were
  created, requested, or transmitted.
- **Report:** `docs/deployment/phase-8-live-wiring-report.md` —
  upstream verification method, profile verification, the
  permission/scope matrix, the measured probe trace (adapter calls
  create→status→archive, NO publish; 2.15 ms in-process; usage
  headroom 95.0%; summary hash 9f8501637be9466a…), rate-limit
  benchmarks, security posture, and the Phase 9 (Telegram Sales &
  Ingress) handover criteria.
- **Verification (2026-09-26):** battery
  `local/tests/test_live_wiring_phase8.py` 45/45 ×2 (pass path +
  deterministic digest, IG-01 refusals with the zero-adapter-call
  proof across four failure classes, profile/seams refusals,
  capability refusals incl. missing scope, expired token, short
  margin, auth error and usage breach, probe refusals incl.
  create failure, retry overflow, terminal state, SAFETY VIOLATION
  publish invocation, state collision and cleanup failure,
  emission contract, redaction scrubs, AST purity) — the phase7
  attestation built by the REAL D-157 igniter over the authentic
  D-156→D-155→D-154 chain; the container workflow through the REAL
...
- **Boundaries preserved:** the channel is verified, not opened —
  ...
  gate: INSTAGRAM_LIVE_ENABLED + env keys), and the D-070 outbox/
...
| 57 | Phase 5 live wiring ignition & service connectivity verification — **D-155 (Approved 2026-09-26, owner-directed)**: `live_wiring_phase5_igniter.py` live-connects the core services under the D-154 certificate — IGN-01 verifies the `dokploy.completion_attestation.v1` (schema, INFRASTRUCTURE_COMPLETE, byte-exact digest recomputation, Stage E manifest binding, D-112 rooting, zero chain breaks) BEFORE any probe runs (absence/tamper/drift/unrooted ⇒ immediate refusal, zero side-effects, NO probes executed); IGN-02 authenticated PostgreSQL SSOT connectivity via the argv-only `ArgvPsqlTransport` (env-name connection refs, ON_ERROR_STOP, strict timeouts, stderr never echoed) + schema readiness (33 tables / 16 schemas / 28 seed rows) + pooling invariants (headroom 12); IGN-03 Redis PING < 50 ms, noeviction, isolated `phase5:ignition_drill` namespace round-trip (REDISCLI_AUTH stays inside the container namespace); IGN-04 n8n dispatcher readiness through the REAL D-053 contracts (schema validation, HMAC sha256= with an engine-invisible secret per D-045, D-027 idempotency drill round-tripping the injected store — the engine's ONLY write); IGN-05 emits exactly ONE canonical `phase5.live_wiring_attestation.v1` per run (aborts included) with the SHA-256 `attestation_digest`. Pure core (RULES §35, AST-pinned), D-151 argv transports (zero shell/Popen), deep redaction (D-124) with public commitments restored. Report `docs/deployment/phase-5-live-wiring-report.md` (topology, latency benchmarks, provenance, Phase 6 handover). Battery `test_live_wiring_phase5.py` 38/38 ×2 over the authentic Stage C→H chain; full regression 1644/1644 ×2 consecutive green across 71 modules (1606 + 38, census reconciled). Live run PHASE5_IGNITED over the authentic certificate against the engine-local stack (Redis PING 28.6 ms). Ignition evidence is not authority — Phases 6–18 and external connectivity remain owner-gated (D-045/D-139,  plan §17/§21.6); no secrets created or transmitted. |
